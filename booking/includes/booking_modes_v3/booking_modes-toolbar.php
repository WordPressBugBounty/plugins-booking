...
function wpbc_booking_modes_enqueue_toolbar_script() {
	...
	$switch_success_message = is_array( $selected_mode )
		? sprintf( __( '%s mode activated.', 'booking' ), $selected_mode['label'] )
		: __( 'Booking Calendar mode activated.', 'booking' );
	...
	wp_localize_script(
		...
		array(
			...
			'i18n'       => array(
				...
				'switch_success'    => $switch_success_message,
			),
		)
	);
}
